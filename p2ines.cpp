    ...
    while (!endTimeStack.empty()) {
        ...
        while (!stack.empty()) {
            ...
            for (int neighbor : graph[vertex].edges) {
                if (graph[neighbor].visited && sccs[neighbor] != sccs[vertex]){
                    distance[vertex] = max(distance[vertex], distance[neighbor] + 1);
                }
                else if(graph[neighbor].visited && sccs[neighbor] == sccs[vertex])
                    distance[neighbor] = max(distance[neighbor], distance[vertex]);
